...
/// Complex search
pub fn post_search(config: ClientConfig, body: types.PostSearchRequestBody) -> Result(response_types.PostSearchResponse, ClientError) {
  let path = "/search"
  ...
  let req = request.set_body(req, encode.encode_post_search_request_body(body))
...
pub fn get_user(config: ClientConfig, user_id: String) -> Result(response_types.GetUserResponse, ClientError) {
  let path = "/users/{userId}"
  let path = string.replace(path, "{userId}", user_id)
  let assert Ok(req) = request.to(config.base_url <> path)
...
/// Receive webhook
pub fn post_webhook(config: ClientConfig, body: types.WebhookEvent) -> Result(response_types.PostWebhookResponse, ClientError) {
  let path = "/webhook"
  let assert Ok(req) = request.to(config.base_url <> path)
  let req = request.set_method(req, http.Post)
  let req = request.set_header(req, "content-type", "application/json")
