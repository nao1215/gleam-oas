...
- Nested external `$ref` inside `ObjectSchema` properties: properties whose value is a relative-file fragment ref (`./other.yaml#/components/schemas/Widget`) are now hoisted — the referenced schema is merged into `components.schemas` under its fragment name, and the property is rewritten to a local ref. Refs inside `array.items`, `additionalProperties`, or composition branches remain unsupported. Nested imports whose fragment name collides with a pre-existing local schema or with another external ref from a different file are surfaced as `Diagnostic` errors instead of silently rebinding to the local slot (#149, subset of #98)
...
- External `$ref` inside `components.request_bodies.*.content.*.schema` and `components.responses.*.content.*.schema`: media-type schemas that point at relative-file fragment refs are now hoisted through the shared `MediaType`-walking helper. Cross-file collision diagnostics span all component kinds via a single imports tracker. External refs pointing at the body / response object itself (the `RefOr.Ref` case) remain out of scope (#159, subset of #98)
- External `$ref` inside `components.parameters.*.content.*.schema`: the `ParameterContent` payload variant now runs through the same `rewrite_media_type_map` helper as request bodies and responses, so parameters declared via content maps get identical collision diagnostics (#161, subset of #98)
- External `$ref` in operation-level schemas: path-level and method-level `parameters`, `requestBody.content.*.schema`, and `responses.<code>.content.*.schema` entries under `paths.*` are now hoisted through the shared helpers. Cross-file collision diagnostics span `components.*` and operation-level imports via a single tracker. External refs pointing at the body / response / parameter object itself (the `RefOr.Ref` case) and refs inside callbacks remain out of scope (#163, subset of #98)
- External `$ref` inside callback PathItems: each callback's `entries` dict is walked via the shared `rewrite_path_item` helper, so inline callback PathItems get their parameters, request bodies, and responses rewritten just like top-level operations. Collision diagnostics span outer operations and inner callbacks via the single imports tracker (#165, subset of #98)
- External `$ref` chained aliases: the external-file resolver now follows one level of alias chaining inside the same file — e.g., `LegacyX: $ref: "#/components/schemas/X"` in `widgets.yaml`, where a consumer imports `LegacyX` — instead of rejecting the target. Cross-file chains collapse transitively because `parse_file` already resolves each loaded file before the outer resolver looks at it. Alias chains longer than one hop inside the same file still surface a diagnostic (#167, subset of #98)
- External `$ref` inside header schemas: both `components.headers.*.schema` and every `Response.headers.*.schema` (under `components.responses` and operation-level responses) now hoist through the shared helpers. Collision diagnostics span header imports via the same tracker as components schemas and operations (#169, subset of #98)
- External `$ref` inside `components.path_items`: reusable PathItem definitions carry the same operations as top-level paths. Their parameters, request bodies, responses, and callbacks are now walked via the shared `rewrite_path_item` helper so external schema refs hoist consistently with `spec.paths` (#171, subset of #98)
